...
def searchShortestPartition(arrOfDot,i,j,numStep=0) :
    ...
    if (j-i == 0) :
        ...
        return calculateDistance(arrOfDot[i], arrOfDot[j]), [i,j], numStep+1 # kembalikan tuple berisi shortest distance dan closest index
        ...
        mid = (i+j)//2
        dist1, closestIndex1, numStep = searchShortestPartition(arrOfDot,i,mid,numStep)
        dist2, closestIndex2, numStep = searchShortestPartition(arrOfDot,mid+1,j,numStep)
        minDist = -1
        closestIndex = [-1,-1]
        if (dist1 <= dist2) :
            minDist = dist1
        ...
        while (p >= i and abs(arrOfDot[p].getCoordinate()[0]-midX) <= minDist) :
            q = mid+1
            while (q <= j and abs(arrOfDot[q].getCoordinate()[0]-midX) <= minDist) : # Sementara masih cek semua titik di dalam strip
                inRange = True
                for dim in range(1, nDim) :
                    if(abs(arrOfDot[q].getCoordinate()[dim]-arrOfDot[p].getCoordinate()[dim]) > minDist) : # Cek apakah ada jarak pada sumbu tertentu yang lebih besar dari minDist
                        inRange = False
                        break
                if (inRange) :
...
def divideAndConquerShortestDistance(listOfDot) :
    ...
    if (len(arrOfDot) > 1) :
        ...
        start_time = time.time()
        shortest_distance, closest_indexes, num_step = searchShortestPartition(arrOfDot,0,listOfDot.getNDots()-1)
    else :
